...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain type"
      decision: "both decision paralysis (don't know what to charge) and workflow friction (manual assembly is slow)"
    - topic: "persona scope"
      decision: "solo freelancer, early career — someone who genuinely doesn't know market rates or how to scope"
    - topic: "insight"
      decision: "LLMs changed the calculus — credible scoped estimates from raw client messages are now possible"
    - topic: "auth model"
      decision: "email + password or OAuth login; standard multi-user web auth"
    - topic: "role model"
      decision: "flat — every account is a freelancer, sees only their own quotes, no admin surface"
    - topic: "MVP flow"
      decision: "7-step flow: sign in → paste inquiry → AI clarifying questions → AI generates line items → user edits → save as draft"
    - topic: "success criteria"
      decision: "primary: inquiry-to-quote < 10 min; secondary: ≥80% AI items need only minor edits; 3-week timeline"
    - topic: "status transitions"
      decision: "all manual — freelancer clicks to change status; no automatic transitions"
    - topic: "FR-008 revision"
      decision: "demoted to nice-to-have — manual line item add deferred to v2"
    - topic: "FR-010 revision"
      decision: "approve concept dropped — one action: save creates/updates quote with status draft"
  frs_drafted: 13
...
- At least 80% of AI-generated line items require only minor corrections before the freelancer approves the quote.
...
- Quote list loads independently of the AI pipeline: CRUD operations on quotes must work without any dependency on AI availability.
...
- **No undo / edit history for line items**: Deletion is permanent. This is an accepted trade-off from the Socratic round (FR-009). The complexity of undo is not justified for MVP.
- **No offline-first guarantee**: The product requires a network connection. No service worker, no local queue, no sync-on-reconnect behavior.
