...
- [ ] 3.4 `<html lang="pl">` w page source
- [ ] 3.5 `<nav aria-label="Nawigacja główna">` widoczny w DevTools
- [ ] 3.6 FormField w błędzie: input ma `aria-invalid="true"` i `aria-describedby`
- [ ] 3.7 Przyciski filtrów QuoteFilterBar mają `aria-pressed`
- [ ] 3.8 Search input ma `aria-label`
- [ ] 3.9 Status selects w QuotesList mają `aria-label` z tytułem wyceny
- [ ] 3.10 Delete buttons mają `aria-label` z tytułem wyceny
- [ ] 3.11 ConversationCard textarea ma `aria-labelledby` wskazujące na pytanie
- [ ] 3.12 UserContextForm textarea ma `aria-label`
...
- [ ] 4.3 QuoteCreator error ma `role="alert"` (inspect)
- [ ] 4.4 QuoteEditor error/success mają `role="alert"` / `role="status"` (inspect)
- [ ] 4.5 UserContextForm error ma `role="alert"` (inspect)
- [ ] 4.6 QuotesList delete error ma `role="alert"` (inspect)
- [ ] 4.7 ConversationCard error ma `role="alert"` (inspect)
- [ ] 4.8 Rollback statusu wyceny → użytkownik widzi komunikat błędu (nie cichy rollback)
...
- [ ] 5.4 `/quotes/nonexistent-id` → komunikat z linkiem powrotu, Topbar widoczny
- [ ] 5.5 Nieistniejący URL → 404.astro z Layout
- [ ] 5.6 Rate limit 429 → komunikat o limicie, nie "błąd połączenia"

### Phase 6: Keyboard Access + Color Contrast

#### Automated

- [ ] 6.1 `npm run build` — bez błędów
- [ ] 6.2 `npm run lint` — bez błędów

#### Manual

- [ ] 6.3 Tab do LineItemsEditor → Enter otwiera edycję komórki
- [ ] 6.4 Spacja na komórce LineItemsEditor → to samo
- [ ] 6.5 Tab przez QuoteFilterBar → focus ring widoczny na każdym przycisku
- [ ] 6.6 Tab przez ConversationCard → focus ring na przycisku "Pomiń"
- [ ] 6.7 Delete buttons mają widoczny kolor w resting state (nie prawie niewidoczny)
